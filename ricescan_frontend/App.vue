...
export default {
  ...
  methods: {
    ...
    captureImage() {
      const canvas = this.$refs.canvas;        // Access the canvas element
      const video = this.$refs.video;          // Access the video element (camera feed)
      const context = canvas.getContext("2d"); // Get the 2d context to draw the video image
      ...
      // Convert data URL to a file object (so it can be uploaded like an image file)
      this.image = this.dataURLtoFile(dataURL, 'captured-image.jpg');

      // Show retry button (if you need it for UI purposes)
      this.showRetryButton = true;

      // Call classify function to classify the rice based on the captured image
      this.classifyRice();
    },

    // Helper method to convert dataURL to file object
    dataURLtoFile(dataURL, filename) {
      const [prefix, base64Data] = dataURL.split(",");
      const mimeType = prefix.split(":")[1].split(";")[0]; // Extract MIME type (e.g., image/jpeg)
      const byteCharacters = atob(base64Data);  // Decode base64 data
      const byteArray = new Uint8Array(byteCharacters.length);

      // Convert the decoded byte characters into byte array
    ...
    ///=== CLASSIFY RICE ===//
    async classifyRice() {
      this.isClassifying = true;   // Flag to show that classification is in progress
      this.loading = true;         // Loading spinner or message
      this.isError = false;        // Reset error flag

      // Prepare the image file for upload (FormData is used to send files in requests)
      const formData = new FormData();
      formData.append('image', this.image);  // Append the image file to the form data

      try {
        // Access the global API URL using this.$apiUrl
        const response = await axios.post(`${this.$apiUrl}/api/classify/`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });

        // Handle the response from the backend
        this.resultMessage = response.data.result;  // Display the result message
        this.riceType = response.data.riceType;     // Display the predicted rice type
        this.resultPopup = true;                    // Show the result popup
        this.loading = false;                       // Hide the loading indicator
      } catch (error) {
        // If an error occurs during classification, show error message
        this.showError("Failed to classify the rice. Please try again.");
      } finally {
        this.isClassifying = false;  // Reset the classifying flag
        this.loading = false;        // Reset the loading flag
      }
    },

    // Error handling method to display an error message
    showError(message) {
      this.isError = true;
